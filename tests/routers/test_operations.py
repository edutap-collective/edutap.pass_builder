...
async def test_refresh_fields_replaces_catalogue(client, session, data_provider):
    ...
    response = await client.post(
        f"{API_PREFIX}/fields/refresh", headers=manager.headers
    )
    assert response.status_code == 200
    ...
    listed = await client.get(f"{API_PREFIX}/fields", headers=manager.headers)
...
async def test_list_fields_requires_manage_scope(client, session):
    ...
    response = await client.get(f"{API_PREFIX}/fields", headers=renderer.headers)
...
async def test_audit_filters_by_outcome(client, session):
    ...
    response = await client.get(
        f"{API_PREFIX}/audit?outcome=error", headers=manager.headers
    )
    assert response.status_code == 200
    [entry] = response.json()
